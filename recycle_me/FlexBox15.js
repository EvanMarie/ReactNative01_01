import React from "react";
import { StyleSheet, Text, View } from "react-native";
import {
  GrandParentFlex,
  GrandParentText,
  InnerText,
  SmallChild01,
  SmallChild02,
  SmallChild03,
} from "./styles_divs/styles";

export const OtherParentFlex = StyleSheet.create({
  backgroundColor: "#6082B6",
  flex: 1,
  flexDirection: "row",
  justifyContent: "center",
  alignItems: "stretch",
});

export const NewChild01 = StyleSheet.create({
  backgroundColor: "#333333",
  width: 75,
});

export const NewChild02 = StyleSheet.create({
  backgroundColor: "#444444",
  width: 75,
  height: 175,
});

const FlexBox15 = () => {
  return (
    <View
      style={{
        height: 350,
        marginBottom: 10,
      }}
    >
      <View style={GrandParentFlex}>
        <Text style={GrandParentText}>
          Flex 15 - GrandParent (flex: 1, padding: 10)
        </Text>
        <Text style={InnerText}>
          flexDirection: "row" justifyContent: "center" alignItems: "stretch"
          (default) - items with no height set will stretch to fill the
          container
        </Text>
        <View style={OtherParentFlex}>
          <View style={NewChild01}>
            <Text style={InnerText}>Child01 - no height</Text>
          </View>
          <View style={NewChild02}>
            <Text style={InnerText}>Child02</Text>
          </View>
          <View style={SmallChild03}>
            <Text style={InnerText}>Child03</Text>
          </View>
        </View>
      </View>
    </View>
  );
};

export default FlexBox15;
